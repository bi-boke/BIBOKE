...
# Function: to start, stop and restart java-bok.
...
JAR_FILE_PATH="./build/libs/java-bok.jar"
PID_FILE_PATH="java-bok.pid"
LOG_FILE_PATH="java-bok.log"
...
case "${1}" in
    start)
        ...
        if [ $? -eq 0 ]; then
            echo "Succeeded to start java-bok."
        else
            echo "Failed to start java-bok."
        fi
    ;;
    stop)
        ...
        if [ $? -eq 0 ]; then
            rm ${PID_FILE_PATH}
            echo "Succeeded to stop java-bok."
        else
            echo "Failed to stop java-bok."
        fi
    ;;
    restart)
