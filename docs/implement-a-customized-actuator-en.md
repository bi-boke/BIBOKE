...
Having a tailored actuator is key to building a java-bok-based customized public chain. This article illustrates how to develop a java-bok-based `SumActuator`.
...
Currently, contracts supported by java-bok are defined under `src/main/protos/core/contract` directory in protocol module. First creating a `math_contract.proto` file under this directory and declaring `SumContract`. You can also implement any mathematical calculation you want, such as `Subtraction`.
...
syntax = "proto3";
package protocol;
option java_package = "org.bok.protos.contract"; //Specify the name of the package that generated the Java file
...
```
At last, recompile the modified proto files. Compiling the java-bok project directly will compile the proto files as well, `protoc` command is also supported.

*Currently, java-bok uses protoc v3.4.0. Please keep the same version when compiling by `protoc` command.*
...
For now, the default Actuator supported by java-bok is located in `org.bok.core.actuator`. Creating `SumActuator` under this directory:
